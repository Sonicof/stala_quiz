...
class StalaQuizApp:
    ...
    def check_credentials(self, username, password):
        try:
            ...
            self.cursor.execute("SELECT usrname, passwd FROM player WHERE usrname=%s AND passwd=%s", (username, hashed_password))
    ...
    def check_username_existence(self, username):
        try:
            if not self.cursor:
                return False
            self.cursor.execute("SELECT usrname FROM player WHERE usrname=%s", (username,))
            data = self.cursor.fetchone()
            return data is not None
        except Exception as e:
    ...
    def create_account(self, username, password, email):
        try:
            ...
            query = "INSERT INTO player (usrname, passwd, email, datee, score, played) VALUES (%s, %s, %s, %s, 0, 0)"
            self.cursor.execute(query, (username, hashed_password, email, datetime.now().strftime("%Y-%m-%d")))
            self.mycon.commit()
